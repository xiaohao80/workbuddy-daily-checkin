...
description: WorkBuddy（腾讯 AI 办公助手）每日自动签到领积分的整套方案：从本机解密登录 Token、调用签到
  API、到设置定时任务/自动化。适用于想"白嫖"Buddy 加油站积分、不再忘记签到的用户。当用户提到 WorkBuddy
  自动签到、每日签到脚本、Buddy 加油站积分自动领取、或想自动化 WorkBuddy 自身功能时使用。
disable: true
---
